...
function detailButtonFormatter(value, row, index) {
	...
	if($('#auditStatus').prop("checked") || $("#readonly").val()=="true") {
		...
		var a2 = '<a class="btn btn-primary" onclick="audit(\'' + row.delayId + '\', true)">同意</a>';
		var a3 = '<a class="btn btn-danger" onclick="audit(\'' + row.delayId + '\', false)">拒绝</a>';
		btns = a2 + a3 + a1;
	}
	return btns;
}
